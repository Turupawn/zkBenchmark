...
template proveWithdrawal(levels) {
    ...
    leaf <== commitmentHasherComponent.commitment; // Divided by ten 
    ...
    for (var i = 0; i < levels; i++) {
        ...
        hashers[i].inputs[0] <== selectors[i].out[0] ** 2;
        hashers[i].inputs[1] <== selectors[i].out[1] ** 2;
        computedPath[i] <== hashers[i].out;

        log("AAAAA");
        log(hashers[i].inputs[0]);
        log(hashers[i].inputs[0]);
        log(computedPath[i]);
    }
    log("Final");
